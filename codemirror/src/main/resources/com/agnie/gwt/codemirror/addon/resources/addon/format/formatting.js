/**
 * NOTE: copied from
 * https://github.com/codemirror/CodeMirror/blob/545aeb74f6800dcaf6acead1bfc14c4eef99c9eb/addon/format/formatting.js
 * 
 * As it is no more supported after 3.1 release hence it has few bugs and issues
 * which are not fixed.
 * 
 */
(function() {
					...
					"xml",
					{
						...
						newlineAfterToken : function(type, content, textAfter,
							...
							if (this.configuration == "html")
								inline = state.context ? inlineElements
										.test(state.context.tagName) : false;
							return !inline
	...
	CodeMirror.defineExtension("commentRange", function(isComment, from, to) {
		var cm = this, curMode = CodeMirror.innerMode(cm.getMode(), cm
				.getTokenAt(from).state).mode;
		cm.operation(function() {
			if (isComment) { // Comment range
				cm.replaceRange(curMode.commentEnd, to);
				cm.replaceRange(curMode.commentStart, from);
				if (from.line == to.line && from.ch == to.ch) // An empty
					// comment
					// inserted -
					// put cursor
					// inside
					cm.setCursor(from.line, from.ch
							+ curMode.commentStart.length);
				...
				if (startIndex > -1 && endIndex > -1 && endIndex > startIndex) {
					...
					selText = selText.substr(0, startIndex)
							...
							+ selText.substring(startIndex
									+ curMode.commentStart.length, endIndex)
							// From comment end till string end
							+ selText.substr(endIndex
									+ curMode.commentEnd.length);
				}
	...
	CodeMirror.defineExtension("autoFormatRange",
			function(from, to) {
				...
				var outer = cm.getMode(), text = cm.getRange(from, to).split(
						"\n");
				var state = CodeMirror.copyState(outer,
